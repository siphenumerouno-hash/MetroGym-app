...
import { Sparkles, Clock, Plus, Trash2, Loader2 } from 'lucide-react';
...
const BuildScreen: React.FC<Props> = ({ store, onStarted }) => {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const suggested = await generateWorkout(goal, type);
      if (suggested.exercises) setExercises(suggested.exercises);
    ...
    <div className="px-6 space-y-8 pb-10">
      ...
      {exercises.length > 0 && (
        <div className="space-y-3">
          {exercises.map((ex, idx) => (
            <div key={ex.id} className="bg-slate-900 rounded-2xl p-4 border border-slate-800 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[8px] font-black text-yellow-400 uppercase tracking-widest">Protocol {idx + 1}</span>
                <button onClick={() => setExercises(exercises.filter(e => e.id !== ex.id))} className="text-slate-700 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
              <input 
      ...
      <div className="bg-slate-900/50 p-6 rounded-3xl space-y-6 border border-slate-800">
        <div className="flex justify-between items-center">
          ...
          <input 
            type="number" 
            className="bg-slate-950 w-16 text-center rounded-lg p-2 text-xs font-black border border-slate-800 text-yellow-400" 
            value={plannedMins} 
